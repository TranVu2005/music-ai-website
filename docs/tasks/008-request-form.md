...
- Components, directories, and files within task scope:
  ...
  - `src/lib/security/rate-limit.ts`: IP rate-limiting utility for submission endpoints. With a single free Render instance (as decided in `docs/decisions/hosting.md`), an in-memory rate limiter is fully acceptable for the Milestone 1 demo. Keep the `RateLimiter` interface with decoupled implementations (`MemoryRateLimiter` and shared-store e.g. `RedisRateLimiter`) selected by environment configuration, ensuring a shared store can be activated effortlessly in the future without modifying application routes.
